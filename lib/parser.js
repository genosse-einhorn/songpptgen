...
define([], function() {
    ...
    function tokenizeAndProcessCommands(content, song) {
        ...
        for (let line of lines) {
            ...
            if (line == '') {
                ...
                // Verse with number as title - treated specially to remove trailing dot
                if (line[line.length-1] == '.')
                    typedLines.push({ type: 'versetitle', text: line.substr(0, line.length-1) });
                else
                    typedLines.push({ type: 'versetitle', text: line });
            } else if (line.match(/^\d+\./)) {
                // EmK-Style Verse
                ...
                if (command == '.') {
                    ...
                    song.mode = args;
                } else if (command.toUpperCase() == 'PART') {
                    typedLines.push({ type: 'versetitle', text: args });
                } else {
    ...
    function parseEmkVerseLine(text) {
        let EMK_VERSE_SPLIT_RE = /[., ]\/ /;
        let EMK_VERSE_PARTS_RE = /[., ]\/\/\/ ?/;

        return text.split(EMK_VERSE_PARTS_RE).map(l => l.split(EMK_VERSE_SPLIT_RE).join('\n'));
    }
    ...
    function parseEmkVerse(lines) {
        ...
        if (nextOfType(lines, 'emkverse')) {
            let line = lines.shift();
            return { title: line.title, parts: parseEmkVerseLine(line.emktext)};
        } else {
            return { title: '', parts: parseEmkVerseLine(lines.shift()) };
        }
    }
    ...
    function detectEmkMode(lines) {
        ...
        // 4th indicator: Emk-Style line splitters are present
