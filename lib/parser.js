...
define([], function() {
    ...
    function parse(content) {
        ...
        for (let i in song.parts) {
            ...
            // split parts that contain split markers HACK: always include terminating newline
            if ((song.parts[i] + '\n').match(SPLIT_MARKER_RE))
                song.parts[i] = (song.parts[i] + '\n').split(SPLIT_MARKER_RE).map(e => e.trim());
        }
