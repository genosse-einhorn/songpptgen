...
define(['../../3rdparty/jszip', '../h'], function(JSZip, h) {
    ...
    function makeXmlShape(shapespec, index, resolveColor, toEmu, toPointPercent) {
        ...
        if (shapespec.type == 'textbox') {
            ...
            shapespec.paragraphs.forEach(function(pspec) {
                ...
                pspec.runs.forEach(function(rspec) {
                    ...
                    lines.forEach(function(line, i) {
                        ...
                        if (rspec.color)
                            rPr.appendChild(genXmlColorSelection(rspec.color, null, resolveColor));
            ...
            return makeXmlShape(nspec, index, resolveColor, toEmu, toPointPercent);
        }
    }

    function makeXmlSlide(slidespec, spec, resolveColor) {
        var intTableNum = 1;
        ...
        h('p:sld', {
            ...
            h('p:cSld', { name: slidespec.name },
                ...
                (slidespec.bgcolor)
                    ? genXmlColorSelection(false, slidespec.bgcolor, resolveColor)
                    : genXmlColorSelection(false, 'FFFFFF', resolveColor),
                ...
                h('p:spTree',
                    ...
                    slidespec.shapes.map(function processShape(shapespec, index) {
                        return makeXmlShape(shapespec, index, resolveColor, toEmu, toPointPercent);
                    })
    ...
    function genXmlColorSelection(color_info, back_info, resolveColor) {
        var ret = [];

        if (back_info) {
            ret.push(h('p:bg',
                h('p:bgPr',
                  genXmlColorSelection(back_info, false, resolveColor),
                  h('a:effectLst')
                )
            ));
        }
        ...
        if (color_info) {
            ret.push(h('a:solidFill',
                h('a:srgbClr', { val: resolveColor(color_info) })
            ));
        }
    ...
    return function render(spec, resolveColor) {
        var zip = new JSZip();
        ...
        for (var idx = 0; idx < spec.pages.length; idx++) {
            ...
            zip.file("ppt/slides/slide"+ intSlideNum +".xml", makeXmlSlide(spec.pages[idx], spec, resolveColor));
